...
AudioFile<double> scatter(AudioFile<double> wav, int splitSize) {
    ...
    for (int channel = 0; channel < 2; channel++){
        for(int i = 0; i < numSplits; i++) {
            buffer[channel].insert(buffer[channel].begin() + i * splitSize, wav.samples[channel].begin() + splitVec[i] * splitSize, wav.samples[channel].begin() + (splitVec[i] + 1) * splitSize);
        }
    }
