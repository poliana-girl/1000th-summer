...
AudioFile<double> sortAscending(AudioFile<double> wav, double length) {
  ...
  for (int split = 0; split < num_splits; split++) {
    double average =
        std::accumulate(
            wav.samples[channel].begin() + (split * length_split),
            wav.samples[channel].begin() + ((split + 1) * length_split), 0.0) /
        length_split;
  ...
  for (int i = 0; i < num_splits; i++) {
    ...
    buffer[channel].insert(
        ...
        wav.samples[channel].begin() + (top_split * length_split),
        wav.samples[channel].begin() + ((top_split + 1) * length_split));
